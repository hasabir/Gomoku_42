...
def place_stone(state, row, col):
    ...
    if state.opening == "pro":
        move_num = len(state.move_history) + 1
        if move_num == 1 and (row, col) != (9, 9):
            state.set_status("Pro: first move must be center (9,9).", error=True)
            return False
        if move_num == 3:
            dist = max(abs(row - 9), abs(col - 9))
            if dist < 3:
                state.set_status("Pro: third move must be 3+ from center.", error=True)
                return False
    ...
    snapshot = {
        ...
        'col': col,
        'player': player,
        'cap_black_before': state.cap_black,
        'cap_white_before': state.cap_white,
    ...
    state.move_history.append(snapshot)
