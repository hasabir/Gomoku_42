...
def draw_opening_screen(surface, fonts):
    surface.fill((20, 18, 14))
    cx = WIN_W // 2
    
    t = fonts['win'].render("OPENING RULE", True, C_TEXT_HI)
    surface.blit(t, (cx - t.get_width() // 2, 80))
    
    sub = fonts['body'].render("Select opening", True, C_TEXT_LO)
    surface.blit(sub, (cx - sub.get_width() // 2, 160))
    ...
    for i, (key, label, desc) in enumerate(options):
        ry = 210 + i * 90
        rect = pygame.Rect(cx - 180, ry, 360, 70)
        pygame.draw.rect(surface, (40, 36, 28), rect, border_radius=10)
        pygame.draw.rect(surface, C_WOOD_DARK, rect, 2, border_radius=10)
        
        key_surf = fonts['big'].render(f"[{key}]", True, C_TEXT_HI)
        surface.blit(key_surf, (rect.x + 18, ry + 8))
        
        lbl_surf = fonts['body'].render(label, True, C_TEXT_WHITE)
        surface.blit(lbl_surf, (rect.x + 18, ry + 32))
...
class GameState:
    ...
    def place_stone(self, row, col):
        ...
        if self.opening == "pro":
            ...
            if move_num == 1 and (row, col) != (9, 9):
                self.set_status("Pro: first move must be center (9,9).", error=True)
                return False
            if move_num == 3:
                dist = max(abs(row - 9), abs(col - 9))
                if dist < 3:
                    self.set_status("Pro: third move must be 3+ from center.", error=True)
                    return False
        ...
        snapshot = {
            ...
            'col': col,
            'player': player,
            'cap_black_before': self.cap_black,
            'cap_white_before': self.cap_white,
            'last_move_before': self.last_move,
            'captured': [],       # filled in below after capture logic runs
        }
        # ── Place the stone ──
    ...
    def undo_move(self):
        ...
        snap = self.move_history.pop()

        # remove the placed stone
        self.board[snap['row']][snap['col']] = 0

        # restore captured opponent stones
        opponent = WHITE if snap['player'] == BLACK else BLACK
        for r, c in snap['captured']:
            self.board[r][c] = opponent

        # restore counts, last_move, turn
        self.cap_black      = snap['cap_black_before']
        self.cap_white      = snap['cap_white_before']
        self.last_move      = snap['last_move_before']
        self.current_player = snap['player']

        # clear win state in case the game had ended
        self.game_over   = False
        self.win_message = ""
        self.set_status("Move undone.", error=False)
        return True
